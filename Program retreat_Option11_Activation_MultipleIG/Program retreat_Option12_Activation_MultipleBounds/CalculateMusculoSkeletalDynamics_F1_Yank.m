function [error] = CalculateMusculoSkeletalDynamics_F1_Yank(q,qd,qdd,lMtilda, lMtilda_init, lM_projected, kFpe, vMtilda, a_ext, a_flex, data_exp, coeff_LMT_ma, params_OS, shift, B, info, Fsrs_del, dFsrs_deldt, kR, kY, tres, Fce_del, dFce_deldt)
...
% FMce 
Fce_ext  = (a_ext + kR * (Fsrs_del + Fce_del) + kY * (dFsrs_deldt + dFce_deldt) - tres).* (FMltilda(1,:).* FMvtilda(1,:)) + Fsrs;      % FMce = fse.* lM ./(lMT-lT) - Fpe;
...
% Total error
error = [error_force error_musclegeometry error_skelet error_Fsrs_del error_Fce_del];
end
