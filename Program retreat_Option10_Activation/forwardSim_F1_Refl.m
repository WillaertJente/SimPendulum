function [q,qd,lMtilda, Fsrs_del] = forwardSim_F1_Refl(q_init,qd_init,lMtilda_init,kFpe,a,data_exp, coeff_LMT_ma, params_OS, shift, dt, N, B, info, Fsrs_del_init, kR);
nMuscles = 2;
nJoints = 1;
...
for i = 1:N
    ...
    dFsrs_deldt_implicit = Urf(3*nJoints+2:3+nMuscles); 
    ...
    Fsrs_del_next = Urf(4*nJoints+nMuscles+2*nJoints+2:4*nJoints+2*nMuscles+2);
    ...
    dFsrs_deldt_implicit_next = Urf(6*nJoints+2*nMuscles+4*nJoints+1:5*nJoints+2*nMuscles+4*nJoints+nMuscles);

    lM_projected = Urf(6*nJoints+2*nMuscles+2*nJoints+nMuscles+2:6*nJoints+2*nMuscles+2*nJoints+2*nMuscles+1);
    lM_projected_next = Urf(6*nJoints+2*nMuscles+2*nJoints+2*nMuscles+2:4*nJoints+2*nMuscles+2*nJoints+4*nMuscles+1);

    
    errorDyn_meshStart = CalculateMusculoSkeletalDynamics_F1_Refl(q(:,i),qd_implicit,qdd_implicit,lMtilda(:,i),lMtilda_init(1), lM_projected,kFpe, vMtilda_implicit,a_ext, a_flex, data_exp, coeff_LMT_ma, params_OS, shift, B, info, Fsrs_del(:,i), dFsrs_deldt_implicit, kR);
    errorDyn_meshEnd = CalculateMusculoSkeletalDynamics_F1_Refl(q_next,qd_next,qdd_implicit_next,lMtilda_next,lMtilda_init(1),lM_projected_next,kFpe, vMtilda_implicit_next,a_ext, a_flex, data_exp, coeff_LMT_ma, params_OS, shift, B, info, Fsrs_del_next, dFsrs_deldt_implicit_next, kR);
    ...
    Fsrs_del(:,i+1) = solution(2*nJoints+nMuscles+2*nJoints+4:4*nJoints+2*nMuscles+2);
    guess = 0.1*ones(3*nStates+2*nMuscles+3,1);
