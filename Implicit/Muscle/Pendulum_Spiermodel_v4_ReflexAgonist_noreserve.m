...
%     Fsrs1        = opti.variable(1,N_1);
    ...
    % Bounds
    opti.subject_to(-4*pi < x   < 4*pi); % Friedl - It seems that these bounds are assuming angles are in degrees, not the case. 
...
%     opti.subject_to(Fsrs1(1) ==0); 
    ...
    opti.subject_to(0.0001 < a_ext - a_ext_0 - Rk*Fsrs_d <0.0001);
...
%     converge.
    ...
    % Dynamics
    xdd = 1/params.I_OS * ((-params.mass_OS*params.g*params.lc_OS*cos(x))+ FT_ext.*ma_ext + FT_flex.*ma_flex - 0.1393*xd); %  + Tdamp + FT*ma);
