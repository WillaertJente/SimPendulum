...
s.tr   = [2 6];                                                             % subject trials (number of trials)
path   = [pathRepo '\Implicit\Muscle\Experimental data\' s.nu '\'];         % Path to opensim model (scaled)
opt    = '_MT6';                                                             % Option used as name to save results
...
% Solutions
Results = WriteSol(result,N_all,N_1_all,s.tr);
  
    sol_x2                 = result(1:N_all(2));                            sol_xd2                = result(N_all(2)+1:2*N_all(2)); 
    sol_lMtilda_ext2       = result(2*N_all(2)+1:3*N_all(2));               sol_lMtilda_flex2      = result(3*N_all(2)+1:4*N_all(2)); 
    sol_Fsrs22             = result(4*N_all(2)+1:4*N_all(2)+N_all(2)-N_1_all(2));  sol_Fsrs_d2            = result(5*N_all(2)-N_1_all(2)+1:6*N_all(2)-N_1_all(2)); 
    sol_a_ext2             = result(6*N_all(2)-N_1_all(2)+1:7*N_all(2)-N_1_all(2));
    sol_lM_projected_ext2  = result(7*N_all(2)-N_1_all(2)+1:8*N_all(2)-N_1_all(2));       sol_lM_projected_flex2 = result(8*N_all(2)-N_1_all(2)+1:9*N_all(2)-N_1_all(2)); 
    sol_act2               = result(9*N_all(2)-N_1_all(2)+1:10*N_all(2)-N_1_all(2));      sol_dt12               = result(10*N_all(2)-N_1_all(2)+1); 
    sol_vMtilda_ext2       = result(10*N_all(2)-N_1_all(2)+2:11*N_all(2)-N_1_all(2)+1);   sol_vMtilda_flex2      = result(11*N_all(2)-N_1_all(2)+2:12*N_all(2)-N_1_all(2)+1); 
    sol_a_ext_02           = result(12*N_all(2)-N_1_all(2)+2);                     sol_a_flex2            = result(12*N_all(2)-N_1_all(2)+3); 
    sol_kFpe_ext2          = result(12*N_all(2)-N_1_all(2)+4);                     sol_kFpe_flex2         = result(12*N_all(2)-N_1_all(2)+5); 
    sol_Rk2                = result(12*N_all(2)-N_1_all(2)+6);
