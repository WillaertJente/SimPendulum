...
% op voorhand lange vectoren met alle frmes
...
%     opti.subject_to(0.05 < kFpe    < 0.15);
    opti.subject_to(0.06 < kFpe_ext    < 0.15);
...
%     opti.set_initial(kFpe,0.1);
    ...
    J            = sumsqr(error)  + sumsqr(error_dot) + 100*sumsqr(act)+ 0.001 * (sumsqr(vMtilda_ext)) ;%+ 100*(q_exp(N_1)-x(N_1));%+ length(q_exp)*0.1*(0.15-kFpe_flex);%+ ...
...
%     sol = opti.solve();
    ...
    sol_x                 = result(1:N);                  sol_xd                = result(N+1:2*N); 
    sol_lMtilda_ext       = result(2*N+1:3*N);            sol_lMtilda_flex      = result(3*N+1:4*N); 
    sol_Fsrs2             = result(4*N+1:4*N+N-N_1);      sol_Fsrs_d            = result(5*N-N_1+1:6*N-N_1); 
    sol_a_ext             = result(6*N-N_1+1:7*N-N_1);
    sol_lM_projected_ext  = result(7*N-N_1+1:8*N-N_1);    sol_lM_projected_flex = result(8*N-N_1+1:9*N-N_1); 
    sol_act               = result(9*N-N_1+1:10*N-N_1);   sol_dt1               = result(10*N-N_1+1); 
    sol_vMtilda_ext       = result(10*N-N_1+2:11*N-N_1+1);sol_vMtilda_flex      = result(11*N-N_1+2:12*N-N_1+1); 
    sol_a_ext_0           = result(12*N-N_1+2);           sol_a_flex            = result(12*N-N_1+3); 
    sol_kFpe_ext          = result(12*N-N_1+4);           sol_kFpe_flex         = result(12*N-N_1+5); 
    sol_Rk                = result(12*N-N_1+6);
    
    save(['C:\Users\u0125183\Box\PhD 1\Simulations Pendulum Test\Results/Result_',char(s.nu),'_T',num2str(s.tr(j)),char(opt),'.mat'],...
        'sol_x', 'sol_xd','sol_lMtilda_ext','sol_lMtilda_flex','sol_Fsrs2','sol_Fsrs_d','sol_a_ext','sol_lM_projected_ext',...
        'sol_lM_projected_flex','sol_act','sol_dt1','sol_vMtilda_ext','sol_vMtilda_flex','sol_a_ext_0','sol_a_flex',...
        'sol_kFpe_ext','sol_kFpe_flex','sol_Rk')
    ...
    subplot(412)
    plot(tvect,sol_Fsrs_d,'LineWidth',1);hold on;
    plot(tvect(N_1+1:end),sol_Fsrs2,'LineWidth',1.5); hold on; 
    box off; ylabel('Fsrs'); legend({'Fsrs_d','Fsrs2'})
    subplot(413)
    plot(tvect,sol_a_ext_0 + sol_Rk*sol_Fsrs_d,'LineWidth',1.5);
    hold on; box off; ylabel('a0+Rk*Fsrs_d');
    subplot(414)
    plot(tvect,sol_act,'LineWidth',1.5); hold on; 
    box off; ylabel('Nm');  legend({'actuator'})
    
    figure(j*1000)
    subplot(131)
    bar(1,sol_a_ext_0); hold on
    bar(2,sol_a_flex); hold on
    box off; legend({'Ext','Flex'}); title('Tone');
    subplot(132)
    bar(1,sol_kFpe_ext); hold on; 
    bar(2,sol_kFpe_flex); hold on; 
    box off; legend({'Ext','Flex'}); title('Passive stiffness');
    axis([0 3 0 0.16])
    subplot(133)
    bar(1,sol_Rk); hold on
    box off; title('Reflex gain(fSRS)')
    axis([0 2 0 4])
