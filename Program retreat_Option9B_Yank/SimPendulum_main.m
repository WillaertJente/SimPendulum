...
info.subj   = 'SDR2_post';           % Subject name
info.trial  = 5;               % Trial number
info.option = 'Opt9_Yank'      % Name to save results
...
Fsrs_del     = opti.variable(1,N); 
Fce_del      = opti.variable(1,N); 

% (Slack) controls
vMtilda      = opti.variable(2,N);
dFsrsdt      = opti.variable(1,N-N_1); 
dFsrs_deldt  = opti.variable(1,N);
dFce_deldt   = opti.variable(1,N); 
...
[error_f1] = CalculateMusculoSkeletalDynamics_F1_Yank(x(1:N_1),xd(1:N_1),xdd(1:N_1), lMtilda(:,1:N_1), lMtilda_init, lM_projected(:,1:N_1),kFpe,vMtilda(:,1:N_1), a_ext, a_flex, data_exp, coeff_LMT_ma, params_OS, shift, B, info, Fsrs_del(1:N_1), dFsrs_deldt(1:N_1), kR, kY, tres, Fce_del(1:N_1), dFce_deldt(1:N_1)); 

% Skeletal dynamics fase 2
[error_f2] = CalculateMusculoSkeletalDynamics_F2_Yank(x(N_1+1:end),xd(N_1+1:end),xdd(N_1+1:end),lMtilda(:,N_1+1:end), lM_projected(:,N_1+1:end), kFpe, vMtilda(:,N_1+1:end), ...
             a_ext, a_flex, data_exp, coeff_LMT_ma, params_OS, shift, B, info, Fsrs, dFsrsdt,Fsrs_del(N_1+1:end), dFsrs_deldt(N_1+1:end), kR, kY, tres,Fce_del(N_1+1:end), dFce_deldt(N_1+1:end) ); 
...
figure()
subplot(221)
plot(R.C.Trefl); hold on
plot(R.C.Tyank); hold on
subplot(222)
bar(1,R.tres);
