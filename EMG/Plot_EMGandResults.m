...
figure()
for t = 1:2
    map_res = ['C:\Users\u0125183\Documents\MATLAB\SimPendulum - programming retreat\Results/',char(subj),'_T',num2str(t),'_Opt7_Weigth_kR_0.5.mat'];
    ...
    box off; xlim([emg(1,1) emg(end,1)+1]); ylim([0 0.006])
    
    subplot(3,5,11)
    bar(k,res.R.a_ext,'FaceColor',color(k,:),'EdgeColor',color(k,:)); hold on
    box off; title('A Ext'); xticks(1:2); ylim([0 0.05])
    
    subplot(3,5,12)
    bar(k,res.R.a_flex,'FaceColor',color(k,:),'EdgeColor',color(k,:)); hold on
    box off; title('A Flex'); xticks(1:2); ylim([0 0.02])
    
    subplot(3,5,13)
    bar(k,res.R.kR,'FaceColor',color(k,:),'EdgeColor',color(k,:)); hold on
    box off; title('kR'); xticks(1:2); ylim([0 0.5])
    
    subplot(3,5,14)
    bar(k,res.R.kFpe,'FaceColor',color(k,:),'EdgeColor',color(k,:)); hold on
    box off; title('kFpe'); xticks(1:2); ylim([0 0.2])
    
    subplot(3,5,15)
    bar(k,res.R.B,'FaceColor',color(k,:),'EdgeColor',color(k,:)); hold on
    box off; title('B'); xticks(1:2); ylim([0 0.15])
    
    k = k+1
end

saveas(gcf,['C:\Users\u0125183\Documents\MATLAB\SimPendulum - programming retreat\Results\Figures/',char(subj),'_EMG_wkR0.5.png'])
saveas(gcf,['C:\Users\u0125183\Documents\MATLAB\SimPendulum - programming retreat\Results\Figures/',char(subj),'_EMG_wkR0.5.fig'])